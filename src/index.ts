import $ from "jquery";
import { Game, Bird, Sprite, Images, Sounds } from "./main";
...
$(document).ready(function() {
    ...
    // sounds
    const sfx_wing = $(".sfx-wing").get(0);
    const sfx_point = $(".sfx-point").get(0);
    const sfx_hit = $(".sfx-hit").get(0);
    const sfx_die = $(".sfx-die").get(0);
    const sfx_swooshing = $(".sfx-swooshing").get(0);
    ...
    const imgs: Images = {
        ...
        splash: splash.get(0),
        thumb: thumb.get(0),
        pipe: [pipe.get(0), pipe_up.get(0), pipe_down.get(0)],
        font_big: [
            font_big_0.get(0),
            font_big_1.get(0),
            font_big_2.get(0),
    ...
    const game = new Game(game_canvas.get(0), imgs, sounds);
